...
export class DruidHelper {
  constructor(druidOptions) {
    ...
    this.maxNumberOfIterationsInScanOffsets =
      (druidOptions && druidOptions.maxNumberOfIterationsInScanOffsets) || 200;
    this.maxTopNThreshold =
      (druidOptions && druidOptions.maxTopNThreshold) || 1000;
    this.druidHelperPrefix = "DruidHelper :: ";
    this.httpCompressData = true;
    if (
      druidOptions &&
      druidOptions.DRUID_API_URI &&
      druidOptions.DRUID_API_URI.length > 0
    ) {
    ...
    if (
      druidOptions &&
      druidOptions.DRUID_QUERY_URI &&
      druidOptions.DRUID_QUERY_URI.length > 0
    ) {
    ...
    if (
      druidOptions &&
      druidOptions.DRUID_USERNAME &&
      druidOptions.DRUID_USERNAME.length > 0
    ) {
    ...
    if (
      druidOptions &&
      druidOptions.DRUID_PASSWORD &&
      druidOptions.DRUID_PASSWORD.length > 0
    ) {
  ...
  ingestSpecFile(specObjectToIngest, options) {
    ...
    return new Promise((resolve, reject) => {
      console.log(
        this.druidHelperPrefix +
          logPrefix +
          " :ingestSpecFile:Info Ingesting file",
      );
      ...
      requestLib.post(postRequestBody, (error, response, body) => {
        if (error) {
          console.log(
            this.druidHelperPrefix +
              logPrefix +
              " :ingestSpecFile:Error in task creation" +
              JSON.stringify(error),
          );
          console.log(
            this.druidHelperPrefix +
              logPrefix +
              " :ingestSpecFile:Error:Response in task creation" +
          ...
          if (options.compact) {
            ...
            if (
              specObjectToIngest.spec.ioConfig.inputSource.uris.length > 0 &&
              specObjectToIngest.spec.ioConfig.inputSource.uris[0] !== ""
            ) {
              if (body && body.task) {
                let taskId = body.task;
                console.log(
                  this.druidHelperPrefix + logPrefix + " :ingestSpecFile: Done",
                );
                resolve(taskId);
              } else {
                console.log(
      ...
      requestLib.get(postRequestBody, (error, response, body) => {
        if (error) {
          console.log(
            this.druidHelperPrefix +
              logPrefix +
              " :checkStatusOfTask:Error in task status check" +
              JSON.stringify(error),
          );
          console.log(
            this.druidHelperPrefix +
    ...
    return new Promise(async (resolve, reject) => {
      if (druidJsonQuery.queryType === "topN") {
        if (druidJsonQuery.filter.fields) {
          let matchedIndex = druidJsonQuery.filter.fields
            .map(function (obj) {
              return obj.type;
            })
            .indexOf("in");
          ...
          if (matchedIndex != -1) {
            if (
              druidJsonQuery.filter.fields[matchedIndex].values.length >
              this.maxTopNThreshold
            ) {
              let inArrayProperties =
                druidJsonQuery.filter.fields[matchedIndex].values;
              let returnedObject = [
                {
                  timestamp: undefined,
                  result: [],
              ...
              for (i = 0, j = inArrayProperties.length; i < j; i += chunk) {
                try {
                  ...
                  let resultsFromThisArray =
                    await this.fetchResults(druidJsonQuery);
                  if (
                    resultsFromThisArray &&
                    resultsFromThisArray[0] &&
                    resultsFromThisArray[0].result
                  ) {
                    returnedObject[0].result = returnedObject[0].result.concat(
                      resultsFromThisArray[0].result,
                    );
                  }
                } catch (error) {
                  console.log(
                    this.druidHelperPrefix + "fetchAllResults",
                    JSON.stringify(error),
                  );
                  reject(error);
                }
          ...
          druidJsonQuery.filter.type &&
          druidJsonQuery.filter.type === "in"
        ) {
      ...
      if (druidJsonQuery) {
        ...
        requestLib.post(postRequestBody, (error, responseData) => {
          if (error) {
            console.log(
              this.druidHelperPrefix +
                "DruidHelper:fetchResults::error: " +
                JSON.stringify(error),
            );
            reject(error);
          } else {
            if (
              ...
              console.log(
                this.druidHelperPrefix +
                  "DruidHelper:fetchResults::statusCode:error: " +
              ...
              if (responseData) {
                if (responseData.body && responseData.body.error) {
                  console.log(
                    this.druidHelperPrefix +
                      "DruidHelper:fetchResults::error: " +
                      JSON.stringify(responseData.body.error),
                  );
                ...
                let errorMessage =
                  this.druidHelperPrefix +
                  "fetchResults::error: No response: " +
                  JSON.stringify(druidJsonQuery);
        ...
        reject({
          errorMessage: "Please define query parameters",
          statusCode: 500,
        });
      }
    });
      ...
      requestLib.post(postRequestBody, (error, responseData) => {
        if (error) {
          ...
          if (
            ...
            console.log(
              this.druidHelperPrefix +
                "fetchResultsSQL::statusCode:error: " +
                JSON.stringify(responseData),
            );
            if (responseData.body && responseData.body.errorMessage) {
              reject({
  ...
  fetchAllResultsSQLScan(druidJsonQuery, offset) {
    //Use it only in Scan and GroupBy queries
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        console.log(
          this.druidHelperPrefix + "fetchAllResultsSQLScan:CountQuery",
          JSON.stringify(error),
        );
        reject(error);
      }
      ...
      if (
        countQueryData &&
        countQueryData[0] &&
        countQueryData[0].numberOfAllRows
      ) {
      ...
      do {
        let druidQueryWithOffset = rebuildQueryWithLimitOffset(
          druidJsonQuery.query,
          offsetForQuery,
          queryLimit,
        );
        try {
          countQueryData = await this.fetchResultsSQL({
            query: druidQueryWithOffset,
          });
        } catch (error) {
          console.log(
            this.druidHelperPrefix +
              "fetchAllResultsSQLScan:FetchQueryWithOffset",
            JSON.stringify(error),
          );
          reject(error);
        }
        ...
        numberOfRequests >= this.maxNumberOfIterationsInScanOffsets
      );

      console.log(
        this.druidHelperPrefix +
          "fetchAllResultsSQLScan: " +
          numberOfAllRows +
          " rows fetched  with " +
          numberOfRequests +
          " requests",
      ...
      function buildCountQuery(sqlQuery) {
        ...
        sqlQuery =
          "SELECT COUNT(*) AS numberOfAllRows FROM" + fromSplitArray[1];
        // Remove group by
  ...
  updateLookupTable(lookupTable, lookupTableJsonConfiguration, options) {
    ...
    return new Promise((resolve, reject) => {
      console.log(
        this.druidHelperPrefix + logPrefix + " :updateLookupTable:Info file",
      );
      const postRequestBody = Object.assign({}, this.postRequestBody, {
        url:
          this.DRUID_API_URI +
          "coordinator/v1/lookups/config/__default/" +
          lookupTable,
      ...
      requestLib.post(postRequestBody, (error, response, body) => {
        if (error) {
          console.log(
            this.druidHelperPrefix +
              logPrefix +
              " :updateLookupTable:Error in lookup creation" +
              JSON.stringify(error),
          );
          console.log(
            this.druidHelperPrefix +
              logPrefix +
              " :updateLookupTable:Error: Response in lookup creation" +
              JSON.stringify(response),
          );
          reject(error);
        } else {
